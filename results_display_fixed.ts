      ...
      {type === 'Fuel' && adjustment.currentAFR && (
        <div className="text-right">
          <p className="text-sm font-mono">
            <span className="text-gray-400">Current:</span> {adjustment.currentAFR} AFR
          </p>
          <p className="text-sm font-mono">
            <span className="text-gray-400">Target:</span> {adjustment.targetAFR} AFR
          </p>
        </div>
      )}
    </div>
    <p className="mt-2 text-gray-200">
      <span className="font-semibold">Suggestion:</span> {adjustment.suggestion}
    </p>
    <p className="mt-1 text-sm text-gray-300">
      <span className="font-semibold">Reason:</span> {adjustment.reason}
    </p>
  </div>
);
...
const ObservationCard: React.FC<{ observation: Observation }> = ({ observation }) => (
  <div className="bg-gray-800/50 border border-gray-700 p-4 rounded-lg">
    ...
    <p className="mt-1 text-raceRed-400">
      <span className="font-bold">Recommendation:</span> {observation.recommendation}
    </p>
  </div>
);
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
            ...
            <XAxis 
              ...
              stroke="#A0AEC0" 
              tick={{ fontSize: 12 }} 
              label={{ value: 'RPM', position: 'insideBottom', offset: -5, fill: '#A0AEC0' }} 
            />
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div>
        ...
        <p className="text-gray-300 leading-relaxed">{suggestions.summary}</p>
      </div>
        ...
        <section>
          ...
          <div className="space-y-4">
            {suggestions.fuelAdjustments.length > 0 ? (
              suggestions.fuelAdjustments.map((adj, index) => (
                <AdjustmentCard key={`fuel-${index}`} adjustment={adj} type="Fuel" />
              ))
            ) : (
        ...
        <section>
          ...
          <div className="space-y-4">
            {suggestions.ignitionAdjustments.length > 0 ? (
              suggestions.ignitionAdjustments.map((adj, index) => (
                <AdjustmentCard key={`ign-${index}`} adjustment={adj} type="Ignition" />
              ))
            ) : (
      ...
      <section>
        ...
        <div className="space-y-4">
          {suggestions.otherObservations.length > 0 ? (
            suggestions.otherObservations.map((obs, index) => (
              <ObservationCard key={`obs-${index}`} observation={obs} />
            ))
          ) : (
